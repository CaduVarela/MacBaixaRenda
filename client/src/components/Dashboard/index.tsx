...
export default function Dasboard() {
  ...
  async function fetchApiStatus() {
    try {
      const response = await fetch(`http://localhost:3001/api/status`).then(
  ...
  async function fetchApiOrders() {
    try {
      const response = await fetch(
        `http://localhost:3001/api/order?filters={"canceled": false}`
  ...
  const handleSaveOrder = async (updatedOrder: IForm) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:3001/api/order/${updatedOrder.id}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
    ...
    <>
      <TableContainer className="scrollbarStyles">
        {orders?.length > 0 ? (
          ...
          <div className="my-10 text-pink font-semibold text-center">
