...
export default function Address({closeModal}: IAddress) {
  ...
  const handleClickSaveAddress = useCallback(
    ...
    [form?.cep, form?.city, form?.neighborhood, form?.number, form?.state, form?.street, showToast]
  );
  ...
  return (
    <div className={styles.containerModal}>
      ...
      <div className="w-full flex items-center justify-evenly">
        ...
        <button onClick={(e) => handleClickSaveAddress(e)} type="submit" className="btnStyles py-1">Adicionar</button>
      </div>
    </div>
  );
}
